...
const InvestmentPanel: React.FC<InvestmentPanelProps> = ({ config }) => {
  ...
  const analyzeOwnedPortfolio = async () => {
    ...
    try {
      ...
      setScanProgress('AI đang chấm điểm & đánh giá ngành...');
...
1. Chấm điểm từng mã trong danh mục (thang điểm 1-10 về độ khỏe dòng tiền và xu hướng kỹ thuật hiện tại).
2. Đánh giá triển vọng NHÓM NGÀNH của từng cổ phiếu trong tầm nhìn 3 THÁNG tới (Tốt / Trung bình / Xấu) và lý do ngắn gọn.
3. Đưa ra lời khuyên hành động realtime (Giữ / Cơ cấu bán / Canh mua thêm).

Yêu cầu trả lời dạng [COLLAPSE: Tên Mã - Điểm số] Nội dung chi tiết bao gồm đánh giá ngành 3 tháng [ENDCOLLAPSE].`;
  ...
  const clearAllData = async () => {
    if (window.confirm("⚠️ BẠN CÓ CHẮC CHẮN MUỐN XÓA TOÀN BỘ DỮ LIỆU ĐẦU TƯ CỦA AI?\n\nHành động này sẽ xóa:\n- Lịch sử chat\n- Các báo cáo vĩ mô, tin tức\n- Dữ liệu sẽ được load lại từ đầu.")) {
      setIsLoading(true);
      try {
        // Clear on Firebase
        await firebaseService.clearAllInvestmentData(userId);
        ...
        // Notify
        setMessages([{ role: 'assistant', content: '✅ Đã xóa toàn bộ dữ liệu hệ thống cục bộ và trên cloud. AI đã được reset.' }]);
      } catch (e: any) {
        console.error('Lỗi khi xóa toàn bộ dữ liệu:', e);
        setMessages(prev => [...prev, { role: 'assistant', content: `❌ Lỗi khi xóa khối dữ liệu: ${e.message}` }]);
      } finally {
        setIsLoading(false);
        setShowPurgeMenu(false);
      }
    ...
    try {
      ...
      for (const rssUrl of rssUrls) {
        try {
          ...
          console.warn(`Lỗi lấy tin từ ${rssUrl}:`, e);
        }
      }
  ...
  const analyzeMacro = async () => {
    ...
    try {
      ...
      const indexText = indexData 
        ...
        : 'Cập nhật: Thị trường đang biến động mạnh, dữ liệu Index đang được load...';

      const prompt = `Hôm nay là ${dateStr}. ${indexText}
Bạn là một Giáo sư Kinh tế học với hơn 10 năm kinh nghiệm thực chiến trên thị trường tài chính Việt Nam. 
Hãy lập báo cáo vĩ mô chuyên sâu dựa trên bối cảnh thực tế hiện tại. Không sử dụng dữ liệu cũ.

Yêu cầu cấu trúc (SỬ DỤNG [COLLAPSE: Tiêu đề] Nội dung [ENDCOLLAPSE] CHO CÁC PHẦN 1, 2, 6, 7):

[COLLAPSE: 1. Toàn cảnh kinh tế vĩ mô & VNINDEX]
(Phân tích sâu về dòng tiền, tâm lý thị trường và tác động của các biến số kinh tế dựa trên con số VNINDEX thực tế ở trên)
[ENDCOLLAPSE]

[COLLAPSE: 2. Chỉ số rủi ro hệ thống]
(Đánh giá rủi ro trên thang điểm 1-5 và lý do)
[ENDCOLLAPSE]

3. Nhóm ngành dẫn sóng (Tầm nhìn 1-3 tháng)
4. Top 5 mã tiềm năng (Ưu tiên giá dưới 60k cho nhà đầu tư cá nhân)

5. Nhóm ngành cần thận trọng

[COLLAPSE: 6. Kế hoạch hành động]
(Lập lộ trình giải ngân hoặc phòng vệ cụ thể cho giai đoạn này)
[ENDCOLLAPSE]

[COLLAPSE: 7. Góc nhìn phản biện]
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="px-4 py-2 bg-cyan-500/5 border-b border-cyan-500/10 flex items-center gap-4 text-[11px] font-mono text-gray-500 shrink-0">
              ...
              <div className="ml-auto flex items-center gap-3 relative">
                {/* Toggle Mode: Scan/Alla - Moved here from header */}
                ...
                >
                 ...
                 {showPurgeMenu && (
                  <div className="absolute top-full mt-1 right-0 bg-[#0a0f19] border border-cyan-500/30 rounded shadow-xl z-50 w-40 py-1 flex flex-col">
                     <button onClick={() => clearChat('1h')} className="px-3 py-1.5 text-left text-[10px] font-mono text-gray-400 hover:text-cyan-400 hover:bg-cyan-500/10 transition-colors">1 GIỜ TRƯỚC</button>
                     <button onClick={() => clearChat('1d')} className="px-3 py-1.5 text-left text-[10px] font-mono text-gray-400 hover:text-cyan-400 hover:bg-cyan-500/10 transition-colors">1 NGÀY TRƯỚC</button>
                     <button onClick={() => clearChat('all')} className="px-3 py-1.5 text-left text-[10px] font-mono text-gray-400 hover:text-cyan-400 hover:bg-cyan-500/10 transition-colors">TOÀN BỘ CHAT</button>
                     <div className="h-px bg-cyan-500/20 my-1 mx-2"></div>
                     <button onClick={clearAllData} className="px-3 py-1.5 text-left text-[10px] font-mono text-red-500 hover:text-red-400 hover:bg-red-500/10 transition-colors">🔥 XÓA EVERYTHING</button>
                  </div>
                )}
              </div>
            </div>
