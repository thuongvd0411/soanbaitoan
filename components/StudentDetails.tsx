...
const StudentDetails: React.FC<Props> = ({ student, onBack, onAddRecord, onEditRecord, onDeleteRecord, hideValues = false }) => {
  const [viewMode, setViewMode] = useState<'history' | 'stats' | 'exam_history'>('stats');
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-10">
      ...
      <div className="flex p-1.5 bg-slate-200/50 rounded-2xl w-fit mx-auto md:mx-0 overflow-x-auto no-scrollbar">
        <button onClick={() => setViewMode('stats')} className={`px-6 md:px-10 py-2.5 md:py-3.5 rounded-xl font-black text-xs md:text-sm transition whitespace-nowrap ${viewMode === 'stats' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-500 hover:text-slate-800'}`}>THỐNG KÊ CHI TIẾT</button>
        <button onClick={() => setViewMode('history')} className={`px-6 md:px-10 py-2.5 md:py-3.5 rounded-xl font-black text-xs md:text-sm transition whitespace-nowrap ${viewMode === 'history' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-500 hover:text-slate-800'}`}>LỊCH SỬ CHI TIẾT</button>
        <button onClick={() => setViewMode('exam_history')} className={`px-6 md:px-10 py-2.5 md:py-3.5 rounded-xl font-black text-xs md:text-sm transition whitespace-nowrap ${viewMode === 'exam_history' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-500 hover:text-slate-800'}`}>LỊCH SỬ LÀM BÀI</button>
      </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm animate-in fade-in duration-500">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="p-4 md:p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Ngày làm</th>
                  <th className="p-4 md:p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tên bài tập</th>
                  <th className="p-4 md:p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Kết quả</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {(!student?.examHistory || student.examHistory.length === 0) ? (
                  <tr><td colSpan={3} className="p-20 text-center text-slate-300 italic font-medium">Chưa có lịch sử làm bài tập</td></tr>
                ) : (
                  student.examHistory.map(ex => (
                    <tr key={ex.id} className="hover:bg-slate-50 transition">
                      <td className="p-4 md:p-6">
                        <div className="font-black text-slate-800 text-sm">{formatDate(ex.date)}</div>
                      </td>
                      <td className="p-4 md:p-6">
                        <div className="font-bold text-slate-700 text-sm">{ex.title}</div>
                        <div className="text-[10px] text-slate-400 font-medium">Mã đề: {ex.id}</div>
                      </td>
                      <td className="p-4 md:p-6 text-center">
                        <div className="inline-block bg-emerald-100 text-emerald-700 px-4 py-1.5 rounded-full font-black text-xs">
